#' @export
player_lists <- function(void){
	
	data(wta_players, package = "oncourt")
	
	url <- "https://api.wtatennis.com/tennis/players/ranked?page=NUM&pageSize=100&type=rankSingles&sort=asc&name=&metric=SINGLES&at=&nationality="
	...
	wta_event_players <- rankings %>%
		dplyr::mutate_all(
			funs(as.character)
		)		
	
	save(wta_event_players, file = "~/Software/inmatch_api_10/data/wta_event_players.RData")
				
	url <- "https://www.atpworldtour.com/en/rankings/singles?rankDate=&rankRange=0-1000&ajax=true"
	...
	atp_event_players <- atp_links %>%
		dplyr::mutate_all(
			funs(as.character)
		)
	
	save(atp_event_players, file = "~/Software/inmatch_api_10/data/atp_event_players.RData")
