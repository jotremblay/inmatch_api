...
#' @export
#'
get_win_prob <- function(data, mens = TRUE){
	
	player1 <- as.character(data$player1[1])
	player2 <- as.character(data$player2[1])
	
	player1.elo <- elo_lookup(player1, mens)
	player2.elo <- elo_lookup(player2, mens)			

	if(length(player1.elo) == 0) player1.elo <- 1800
	if(length(player2.elo) == 0) player2.elo <- 1800
	...
	player1.serve.prob <- calibrate_serve(player1.win.prediction, atp = mens)

	player2.serve.prob <- calibrate_serve(player2.win.prediction, atp = mens)
